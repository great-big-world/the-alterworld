        ...
        if (!canGenerate(structureWorldAccess, blockPos)) {
            ...
            for (int l = 0; l < k; ++l) {
                ...
                for (Direction direction : DIRECTIONS) {
                    if (structureWorldAccess.getBlockState(blockPos3).isAir() && structureWorldAccess.getBlockState(blockPos3.offset(direction)).isSideSolidFullSquare(structureWorldAccess, blockPos3.offset(direction), direction.getOpposite())) {
                        structureWorldAccess.setBlockState(blockPos3, Blocks.SCULK_SHRIEKER.getDefaultState().with(SculkShriekerBlock.CAN_SUMMON, true).with(Properties.FACING, direction.getOpposite()), 3);
                    }
                }
            }
